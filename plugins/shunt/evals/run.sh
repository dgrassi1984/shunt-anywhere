...
#
# Usage:
#   bash evals/run.sh              # hooks only (no worker CLI needed)
#   bash evals/run.sh --benchmark  # hooks + token savings (needs a signed-in worker CLI)
...
setup_fixtures() {
  ...
  for ((i = 0; i < count; i++)); do
    # Entries whose paths the single-path parser cannot reach — a batch read, a
    # path under a shell -c wrapper, an argv array — declare their files
    # outright: "fixtures": [{"path": "...", "lines": N}, ...]
    local nf j
    nf=$(jq -r ".evals[$i].fixtures // [] | length" "$evals_file")
    for ((j = 0; j < nf; j++)); do
      local fpath flines
      fpath=$(jq -r ".evals[$i].fixtures[$j].path" "$evals_file" | sed "s|{{FIXTURES}}|$FIXTURES|")
      flines=$(jq -r ".evals[$i].fixtures[$j].lines" "$evals_file")
      case "$fpath" in "$FIXTURES"/*) generate_fixture "$fpath" "$flines" ;; esac
    done
...
run_eval() {
  ...
  # A hook allows by exiting 0 and blocks by exiting 2 with the reason on
  # stderr. That pair is the one refusal Claude Code, Codex and Gemini CLI all
  # honour, so the decision is the exit code, not a parsed envelope.
  local rc actual
  if [ -n "$env_json" ] && [ "$env_json" != "null" ]; then
    ...
    echo "$input" | env $env_cmd "${BASH:-bash}" "$hook" >/dev/null 2>&1 && rc=0 || rc=$?
  else
    echo "$input" | "${BASH:-bash}" "$hook" >/dev/null 2>&1 && rc=0 || rc=$?
  fi
  case "$rc" in
    0) actual="allow" ;;
    2) actual="block" ;;
...
# Runs as a child process: the suite puts stub CLIs on PATH, and those must not
# leak into the benchmarks below, which need the real worker.
run_transport_suite() {
  echo ""
  echo "Transport (scripts/lib/worker.sh, stubbed worker CLIs)"
  echo "────────────────────────────────────────────────────────────────"
...
run_benchmarks() {
  ...
  if ! shunt_preflight; then
    printf "\n\033[33mSkipping benchmarks: no worker CLI or jq available\033[0m\n"
    return
  fi
